...
void ld_l2miss(int * p, volatile int iterations)
{
    ...
    //Each line of the cache is 32 bytes
    //Each load loads a different line of the cache
    //L2 cache 4 ways and 65KB each, total 256KB, 32 bytes per set, so 2048 lines/sets
    //65KB/32KB = 2048 lines    
    //
    //This function load every 32 bytes, 64 times
    //Then 2048sets x 5ways = 10240 sets

   
 int *q;
 int reg;
 int block_set = 160;   // 10240/64 = 160 (all sets/half way)
 //volatile int iterations_int = iterations;

 __asm__ __volatile__ (".ldl2miss_begin:"    "\n\t"
 "add %5,%1, 0" "\n\t" 
 ".ldl2missloop_begin:"  "\n\t"
 "ld %0, 0*32(%1) " "\n\t"
 "ld %0, 1*32(%1) " "\n\t"
 ...
 "add %1,%1, %2" "\n\t"
 "addi %4,%4, -1" "\n\t"
 "bne  %4, zero, .ldl2missloop_begin"  "\n\t"
 "addi %4,%4, 160" "\n\t"
 "add %1,%5, 0" "\n\t" 
 "addi %3,%3, -1" "\n\t"
 "bne  %3, zero, .ldl2missloop_begin"  "\n\t"
 "nop"  "\n\t"
 ".ldl2miss_end:"   "\n\t"
 ://%0        %1      %2          %3               %4              %5
 : "r"(reg), "r"(p), "r"(64*32), "r"(iterations), "r"(block_set), "r"(q)
