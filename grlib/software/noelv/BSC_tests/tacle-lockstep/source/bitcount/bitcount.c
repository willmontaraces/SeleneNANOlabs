...
int __attribute__ ((noinline)) bitcount_return()
{
  return ( bitcount_n + ( -1095 ) ) != 0;
}

void __attribute__ ((noinline)) bitcount_init()
{
  bitcount_randseed = 1;
  bitcount_n = 0;
...
void __attribute__ ((noinline)) bitcount_main()
{
  unsigned int i, j;
