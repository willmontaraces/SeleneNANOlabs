...
void  __attribute__ ((noinline)) iir_init( void )
{
  int f;
  unsigned int i;
  unsigned char *p;
...
int  __attribute__ ((noinline)) iir_return( void )
{
  float checksum = 0.0;
  int f;
...
void  __attribute__ ((noinline)) iir_main( void )
{
  register float w;
  int f;
...
int main(void)
{
    ...
    switch (__CORE__) {
        case 1: 
            ...
            if ( (iir_return() - 400 != 0)  == 0 ) 
                printf(" PASSED\n");
            else
                printf(" FAILED\n");

            printf("Result_from_CORE2:");
            if ( (*shared_result - 400 != 0) == 0 )
                printf(" PASSED\n");
            else
                printf(" FAILED\n");
