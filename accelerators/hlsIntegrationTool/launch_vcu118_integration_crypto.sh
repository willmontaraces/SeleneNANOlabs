...
with open('vcu118/script.tcl', 'w') as filename_out:  # Outfile name changed
    with open(str(hls_solution_name) + '/script.tcl', 'r') as filename_in:
...
#-----End Create new solution folder--------------------------------------------------------
...
./pyscript2.py ${hls_solution_name} ${hls_project_solutions_dir}
CHECK=$(ls | grep "pyscript2.py" | wc -l)
if [ $CHECK -ne "0"  ]; then
    rm pyscript2.py
fi
